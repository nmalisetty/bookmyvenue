...
export default () => {
    ...
    async function getUpcomingReservations() {
        let response = await fetch('http://localhost:6969/reservations/availability/venues/host/' + sessionStorage.getItem('user_name'), {
            method: 'GET'
        });
    ...
    async function cancelReservation(reservation_id) {
        let response = await fetch('http://localhost:6969/reservations/reservation/cancel/' + reservation_id, {
            method: 'GET'
        });
        let jsonResponse = await response.json();
        if (jsonResponse.status) {
